...
Page({
  ...
  saveimgpath :function(temppath){
    console.log('saveimgpath',temppath)
    let pages = getCurrentPages();
    let prevPage = pages[pages.length-2]; 
    if(prevPage.data.wimg == 0){
      prevPage.setData({
        imgpath0 : temppath,
      });
      console.log('save success!',temppath);
   }
   else{
    prevPage.setData({
      imgpath1 : temppath,
  ...
  //完成，存储在文件夹中~
  //为了达到旋转的目的，仅仅能旋转图片，因为画布旋转势必导致原来的也转
  subCanvas() {
